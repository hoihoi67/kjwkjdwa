...
const unicodeEmojiMap = {
  logo2: '🛒', statystyki: '📊', solana: '🪙', skrill: '💳', serce: '❤️', revolut: '💸', regulamin: '📜', pytanie: '❓', puchar: '🏆', psc: '📮', kudka: '🎁', gift: '🎉', powitanie: '👋', portfel: '💼', pin: '📌', pp: '🪪', osoby: '👥', osoba: '👤', nitro: '🚀', nie: '❌', middleman: '🤝', megafon: '📣', lupa: '🔎', ltc: '📈', like: '👍', zarowka: '💡', kursor: '🖱️', ksiazka: '📚', kosz: '🗑️', konkurs: '🏅', kasa: '💰', kalendarz: '📅', info: 'ℹ️', gwiazdka: '⭐', folder: '📁', eth: 'Ξ', dzwonek: '🔔', kredka1: '✏️', dislike: '👎', dc: '💻', gwiady: '✨', data: '🗓️', zegar: '⏰', tickety: '🎫', blik: '⚡', crypto: '₿', boost: '🚀', przecena: '🏷️', strzalka: '➜',
};
const ce = (name) => ({ name: unicodeEmojiMap[name] ?? '✨' });
const et = (name) => unicodeEmojiMap[name] ?? '✨';
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === "drop-panel") {
      await sendDropPanel(interaction.channel);
...
async function sendTicketPanel(channel) {
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
    new TextDisplayBuilder().setContent(`> ${et("logo2")} © 2026 n1tr0 market × Panel Ticketów`),
    );
...
async function sendCennikPanel(channel) {
  ...
  if (categories.length) {
    menu.addOptions(
      categories.slice(0, 25).map(([key, item]) => ({
        ...
        emoji: getCennikEmoji(item.emoji),
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          ...
          "",
          "➜ ・ **Wybierz kategorię** produktu, która Cię **interesuje!**",
        ].join("\n"),
      ),
    )
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Oferta`),
    );
...
function getCennikEmoji(value) {
  const allowed = new Set(Object.values(unicodeEmojiMap));
  return typeof value === "string" && allowed.has(value) ? value : et("logo2");
}
...
async function handleCennikCommand(interaction) {
  ...
  if (subcommand === "panel") {
    try {
      await sendCennikPanel(interaction.channel);
    } catch (error) {
      console.error("Nie udalo sie wyslac panelu cennika:", error);
      throw error;
    }
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          header,
          header ? "" : null,
          `\`\`\` ${getCennikEmoji(item.emoji)} ・ n1tr0 market × ${item.title || item.label}\`\`\``,
          "",
...
function createLegitCheckTicketPanel(clientUser, sellerMember, legitText) {
  ...
  return panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Legit Check`),
    );
}
...
async function sendRulesPanelCommand(interaction) {
  await sendRulesPanel(interaction.channel);
  await interaction.reply({
...
async function sendRulesPanel(channel) {
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("serce")} © 2026 n1tr0 market × Regulamin`),
    );
...
function createRulesPagePanel(pageIndex) {
  ...
  return new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          `\`\`\` 🖤 ・ Regulamin n1tr0 market - Strona ${pageIndex + 1}/${rulesPages.length}\`\`\``,
          "",
...
async function sendVerificationPanel(channel) {
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("serce")} | © 2026 n1tr0 market × Weryfikacja`),
    );
...
async function sendOpinionPanel(channel) {
  ...
  const panel = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          "``` 🖤 ・ n1tr0 market × WYSTAW OPINIĘ```",
          "",
          "➜ ・ Chcesz podzielić się **opinią** o naszym **serwerze?**",
          "➜ ・ **Będziemy mega wdzięczni** za **wystawienie nam** opinii - buduje to **zaufanie** do nas!",
          "➜ ・ **Kliknij** przycisk poniżej, aby wystawić opinię.",
        ].join("\n"),
      ),
    )
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    ...
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("© 2026 n1tr0 market × OPINIE"),
    );
...
async function deleteLegitStickyPanels(channel) {
  ...
  const panelMessages = messages.filter((message) =>
    message.author.id === client.user.id &&
    message.components.some((component) => hasComponentText(component, "n1tr0 market × LEGIT CHECK")),
  );
...
async function sendLegitReactionPanel(channel) {
  const yesEmoji = config.reactionYesEmoji || '✅';
  const noEmoji = config.reactionNoEmoji || '❌';
  ...
  const panel = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          "``` 💎 ・ n1tr0 market × CZY JESTEŚMY LEGIT```",
          "",
          ...
          "",
          `➜ ・ **Wystawienie reakcji** ${et("nie")} **bez dowodu** skutkuje **natychmiastową przerwą na 7 dni!**`,
        ].join("\n"),
      ),
    );
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Zaznacz Reakcję`),
    );
...
async function handleRestockMessage(message) {
  ...
  await sendRestockPanel(message.channel, message.member, content);
}

async function sendRestockPanel(channel, member, content) {
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Restock`),
    );
...
async function sendBlacklistPanel(channel, entry) {
  ...
  const panel = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          "``` 🖤 ・ n1tr0 market × BLACKLISTA```",
          "",
          `? ・ ${et("osoba")} **Użytkownik:** ${entry.target}`,
          `? ・ ${et("dc")} **ID:** \`${entry.target.id}\``,
          `? ・ ${et("data")} **Data Dodania:** \`${displayDate}\``,
          `? ・ ${et("info")} **Powód:** \`${entry.reason.replace(/`/g, "'").slice(0, 900)}\``,
        ].join("\n"),
      ),
    )
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `${et("logo2")} © 2026 n1tr0 market × Blacklista | Wystawione przez: ${entry.moderator.username}`,
      ),
    );
...
function isOpinionResultMessage(message) {
  return message.author.id === client.user.id &&
    message.embeds.some((embed) => embed.description?.includes("n1tr0 market × OPINIE"));
}
...
function isYesReaction(reaction) {
  const yesEmoji = config.reactionYesEmoji || '✅';
  return reaction?.emoji?.name === yesEmoji || reaction?.emoji?.name === 'yes';
}

function resolveUnicodeEmoji(name, fallback = '✨') {
  return unicodeEmojiMap[name] || fallback;
}
...
async function sendDropPanel(channel) {
  const buttons = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Wylosuj Zniżkę`),
    );
...
async function showInviteStats(interaction) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({ text: `n1tr0 market · Zapytane przez ${interaction.user.username} · ${formatShortDate(new Date())}` });
...
async function sendInviteJoinMessage(member, inviteInfo) {
  ...
  const panel = new ContainerBuilder()
    .setAccentColor(config.panelColor)
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent("``` 💎 ・ n1tr0 market × ZAPROSZENIE```"),
    )
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
  ...
  panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("serce")} © 2026 n1tr0 market × Zaproszenie`),
    );
...
async function createGiveaway(interaction) {
  ...
  const message = await interaction.channel.send({
    components: [createGiveawayPanel({ prize, winnersCount, endsAt, participantsCount: 0, giveawayId: "pending" })],
    flags: MessageFlags.IsComponentsV2,
  });
  ...
  await message.edit({
    components: [createGiveawayPanel({ prize, winnersCount, endsAt, participantsCount: 0, giveawayId: message.id })],
    flags: MessageFlags.IsComponentsV2,
  });
...
function createGiveawayPanel({ prize, winnersCount, endsAt, participantsCount, giveawayId, disabled = false, winners = [] }) {
  ...
  const panel = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        [
          "``` 💎 ・ n1tr0 market × KONKURS```",
          "",
          `> ? ・ **Nagrodą W Konkursie Jest:** \`${prize}\``,
          `> ? ・ **Nagrodę Może Wygrać:** \`${winnersCount} ${formatPolishPeople(winnersCount)}\``,
  ...
  return panel
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("kudka")} © 2026 n1tr0 market`),
    );
}
...
function scheduleStoredGiveaways() {
  ...
  for (const giveaway of Object.values(data.giveaways)) {
    ...
    if (delay <= 0) {
      endGiveaway(giveaway.id).catch((error) => console.error("Wypierdoliło Błąd sprawdz konsole:", error));
    } else {
...
async function showDropInventory(interaction) {
  ...
  const description = [
    ...
    "",
    `-# ${et("logo2")} © 2026 n1tr0 market ∿ Ekwipunek`,
  ].join("\n");
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      [
        ...
        "",
        `-# ${et("logo2")} © 2026 n1tr0 market ∿ Drop`,
      ].join("\n"),
    );
}
...
async function createTicket(interaction) {
  ...
  try {
    ...
    console.error("Nie udalo sie wyslac pierwszej wiadomosci ticketu:", error);
    await interaction.editReply(
      "Ticket został utworzony, ale nie udało się wysłać pierwszej wiadomości. Sprawdź uprawnienia bota na tej kategorii.",
    );
    return;
  }
...
function createTicketOpenPanel(user, displayName, typeLabel, answerLines, mentionLine) {
  ...
  return new ContainerBuilder()
    ...
    .addSeparatorComponents(new SeparatorBuilder().setDivider(true))
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(`${et("logo2")} © 2026 n1tr0 market × Ticket`),
    );
}
